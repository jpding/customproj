...
function onAfterReportCalc(args) {
    ...
    var startCol = 31;//getStartCol(table);
    if(startCol == -1){
...
//    	
    	...
    	if(vv == '交船'){
    		if(i%2==0){
    			...
    			println("交船：startCell:"+startCell.getName()+";value="+startCell.getValue());
    			println("交船：endCell:"+endCell.getName()+";value="+endCell.getValue());
...
function getEndTimeCellValue(table, cell, dataStartCol){
	...
	var currV = "";
	for(var i=endcol; i<COL_DATASTART; ){
...
function getStartCell2(table, cell, dataStartCol, sjq, sjz){
	var startcol = getColIndex(COL_STARTTIME) - 1;
	var currV = "";
	println("getStartCell2:startcol:"+startcol+";row:"+cell.row+";dataStartCol:"+dataStartCol);
	for(var i=startcol; i<COL_DATASTART; ){
...
function drawNoBorder(table, startCell){
	for(var i=32; i<table.colCount; i++){
...
function drawMileStone(table, startCell, endCell, dataStartCol, startTimeCol, endTimeCol, startCol){
	...
	var cellContents = [];
	var cellValues = [];
	for(var i=start+1; i<startCol+2; ){
		var cellKey = table.getCell(startCell.row, i);
		cellContents.push(cellKey);
		if(cellKey){
			cellValues.push(cellKey.value);
		}
		i += 2;
	}
	
	println("cellContents:"+cellValues.join(";"));
